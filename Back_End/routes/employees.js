...
// GET all employees (only this user's)
...
// POST create employee (tagged to this user)
router.post('/', async (req, res) => {
  try {
    ...
    const emp = await Employee.create({
      userId: req.user.id,
      name, role, dept, email, phone,
      salary: Number(salary),
      status, joined, avatar,
    });
...
// PUT update employee (only if it belongs to this user)
...
// DELETE employee (only if it belongs to this user)
